...
<div class="py-3 max-w-xl">
<h2 class="text-base sm:text-lg font-bold text-gray-800 dark:text-white mb-4">Profil Admin</h2>

<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
    <div class="h-1 bg-gradient-to-r from-indigo-500 to-violet-500"></div>
    <div class="p-5 sm:p-6">
        <?= form_open_multipart('/admin/profile/update', ['class'=>'space-y-5']) ?>
        <?= csrf_field() ?>
        ...
        <div class="flex items-center gap-4" x-data="imgPrev()">
            <div class="relative group cursor-pointer flex-shrink-0" @click="$refs.fi.click()">
                <img :src="preview||'<?= base_url('uploads/profiles/'.($user['foto_profil']??'default.png')) ?>'"
                     class="w-20 h-20 rounded-full object-cover ring-4 ring-indigo-100 dark:ring-indigo-900/30"
                     onerror="this.src='https://ui-avatars.com/api/?name=<?= urlencode($user['nama_lengkap']) ?>&background=6366f1&color=fff&size=80'">
                <div class="absolute inset-0 bg-black/40 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <i class="fas fa-camera text-white text-lg"></i>
                </div>
            </div>
            <div>
                <p class="text-sm font-semibold text-gray-700 dark:text-gray-300"><?= esc($user['nama_lengkap']) ?></p>
                <p class="text-xs text-gray-400 mt-0.5">@<?= esc($user['username']) ?></p>
                <button type="button" @click="$refs.fi.click()" class="mt-2 text-xs text-indigo-500 hover:text-indigo-600 font-medium underline">Ganti foto profil</button>
                <p class="text-[10px] text-gray-400 mt-0.5">JPG/PNG/GIF, maks. 2MB</p>
            </div>
            ...
            <div x-data="{show:false}">
                ...
                <div class="relative">
                    <input :type="show?'text':'password'" name="password" placeholder="Kosongkan jika tidak diubah"
                           class="w-full h-11 px-4 pr-11 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder:text-gray-400">
        ...
        <button type="submit"
                class="w-full sm:w-auto h-11 px-8 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800
                       text-white font-semibold text-sm rounded-xl flex items-center justify-center gap-2 transition-colors shadow-sm">
